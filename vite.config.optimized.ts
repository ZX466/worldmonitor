...
const BROBTLI_EXTENSIONS = new Set(['.js', '.mjs', '.css', '.html', '.svg', '.json', '.txt', '.xml', '.wasm']);
  ...
  return {
    ...
    async writeBundle(outputOptions, bundle) {
      ...
      await Promise.all(Object.keys(bundle).map(async (fileName) => {
        ...
        if (!BROBTLI_EXTENSIONS.has(extension)) return;

        const sourcePath = resolve(outDir, fileName);
        const compressedPath = `${sourcePath}.br`;
        const sourceBuffer = await readFile(sourceBuffer);
    ...
    VitePWA({
      ...
      manifest: {
        ...
        icons: [
          ...
          { src: '/favico/android-chrome-512x512.png', sizes: '512x'512', type: 'image/png' },
