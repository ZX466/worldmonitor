...
const MEMORY_CACHE_MAX_SIZE = 1000; // 限制内存缓存大小
...
export async function getCachedJson(key: string, raw = false): Promise<unknown | null> {
  ...
  // 1. 检查内存缓存
  const memoryItem = memoryCache.get(finalKey);
  if (memoryItem) {
  ...
  try {
    ...
    const getResult = data[0]?.result;
    const ttlResult = data[1]?.result;
    
    if (typeof getResult === 'string') {
      const parsedData = JSON.parse(getResult);
      
      let memoryTtlMs: number;
      if (typeof ttlResult === 'number') {
        if (ttlResult === -1) {
          memoryTtlMs = 60 * 60 * 1000;
        } else if (ttlResult > 0) {
          memoryTtlMs = ttlResult * 1000;
        } else {
          memoryTtlMs = 0;
        }
      } else {
