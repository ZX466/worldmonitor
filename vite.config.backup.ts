...
// RSS代理域名白名�?const RSS_PROXY_ALLOWED_DOMAINS = new Set([
    ...
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        ...
        try {
          ...
          res.setHeader('Content-Type', 'application/json');
          res.end(JSON('application/json', { error: error.name === 'AbortError' ? 'Feed timeout' : 'Failed to fetch feed' }));
        }
      });
    },
  };
...
// 获取环境变量配置的端�?const serverPort = parseInt(process.env.VITE_SERVER_PORT || '3000', 10);
