...
export class MapComponent {
  ...
  private updateLayersByZoom(): void {
    const zoom = this.state.zoom;
    const layerVisibility: Partial<MapLayers> = {};
    
    // Define zoom thresholds for each layer
    const zoomThresholds: Partial<Record<keyof MapLayers, number>> = {
      bases: 3,
      nuclear: 2,
      datacenters: 3,
      stockExchanges: 3,
    ...
    Object.entries(zoomThresholds).forEach(([layer, minZoom]) => {
      const layerKey = layer as keyof MapLayers;
      if (zoom >= minZoom) {
    ...
    Object.entries(layerVisibility).forEach(([layer, visible]) => {
      const layerKey = layer as keyof MapLayers;
      if (this.state.layers[layerKey] !== visible) {
        this.state.layers[layerKey] = visible as boolean;
        this.onLayerChange?.(layerKey, visible as boolean, 'programmatic');
      }
    });
  ...
  private setupZoomHandlers(): void {
    ...
    this.container.addEventListener(
      ...
      (e) => {
        ...
        if (Math.floor(this.state.zoom) !== Math.floor(lastZoom)) {
          this.updateLayersByZoom();
